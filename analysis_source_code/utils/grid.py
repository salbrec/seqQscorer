"""Grid search algorithms and parameter settings
...
import utils.MyXGBoost as MyXGBoost
...
# define the whole grid and return the corresponding variables
def get_grid():
	SEED = 1
	...
	algorithms['SVC'] = SVC(kernel='rbf', probability=True, 
						 random_state=the_RS)
	...
	param_grid['MLP'] = {'solver': ['lbfgs', 'sgd'], 
						...
						'hidden_layer_sizes':[(5,) , (10,) , (50,), (100,) , 
							(5,5), (10,10), (50,10), (100,50), (200,100), 
							(500,400)]}
	...
	algorithms['XGB'] = MyXGBoost.XGBoostClassifier(eval_metric = 'auc',
								num_class = 2, nthread = 4, silent = 1)
	param_grid['XGB'] = {'num_boost_round': [100, 250, 500],
						'eta': [0.05, 0.1, 0.3],
						'max_depth': [6, 9, 12],
						'subsample': [0.9, 1.0],
						'colsample_bytree': [0.9, 1.0]}
	param_grid_test['XGB'] = {'num_boost_round': [50], 
							'max_depth': [6]}

	# AdaBoost with DTL
	classifiers.append('ADT')
	DTC = DecisionTreeClassifier(random_state = the_RS, max_features = "auto", 
